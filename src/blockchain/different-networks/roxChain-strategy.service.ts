...
@Injectable()
export class RoxChainStrategyService implements IBlockChainPrivateServer {
    ...
    async createWallet(): Promise<IWalletKeys> {
        const roxWallet = Keypair.generate();
        const privateKey = Buffer.from(roxWallet.secretKey).toString('base64');
        const address = roxWallet.publicKey.toBase58();
    ...
    async getSignedTransaction(
        ...
        try {
            ...
            switch (this.asset.type) {
                ...
                default:
                    throw new BadRequestException(
                        `Unsupported asset type for roxChain: ${this.asset.type}`,
                    );
            }
            ...
            this.logger.error(`RoxChain Transaction Signing error: ${error?.stack ?? error?.message}`);
            return {
                bundlerUrl: this.host,
    ...
    async getSignedTransactionCoin(
        ...
        return await this.signAndReturnRoxChainTransaction(transaction, feePayer, sender);
    }
    ...
    private async signAndReturnRoxChainTransaction(transaction: Transaction, feePayer: Keypair | null, sender: Keypair) {
    ...
    async getSignedSwapTransaction(
        ...
        throw new Error('RoxChain does not support swap transactions.');
    }
}
